...
public:
    ...
    static TlsHello getDefault() {
        ...
        // PR #30738: Randomize cipher suites to vary the TLS fingerprint.
        // Always include TLS 1.3 ciphers, then add a random subset from a pool.
        static const char *cipherPool[] = {
                "\xc0\x2b", "\xc0\x2f", "\xcc\xa9", "\xcc\xa8",
                "\xc0\x2c", "\xc0\x30", "\xc0\x14", "\x00\x9c",
                "\x00\x9d", "\x00\x2f", "\x00\x35", "\xc0\x13",
                "\x00\x3c"
        };
        static const int poolSize = sizeof(cipherPool) / sizeof(cipherPool[0]);

        // Always start with the TLS 1.3 ciphers
        std::string cipherData;
        cipherData.append("\x13\x01", 2);
        cipherData.append("\x13\x03", 2);
        cipherData.append("\x13\x02", 2);

        // Pick a random number of extra ciphers (1-9)
        uint32_t extraRnd;
        RAND_bytes(reinterpret_cast<unsigned char *>(&extraRnd), sizeof(extraRnd));
        int extra = 1 + (int)(extraRnd % 9);

        // Fisher-Yates shuffle the pool indices
        int indices[13];
        ...
        for (int i = 0; i < extra && i < poolSize; i++) {
            cipherData.append(cipherPool[indices[i]], 2);
        }

        // Cipher suites list length = grease(2 bytes) + cipherData
        uint16_t cipherLen = (uint16_t)(2 + cipherData.size());
        std::string lenStr;
        lenStr.push_back((char) ((cipherLen >> 8) & 0xFF));
        lenStr.push_back((char) (cipherLen & 0xFF));
        ...
        res.ops = {
                ...
                Op::string("\x20", 1),
                Op::random(32),
                Op::string(lenStr.data(), lenStr.size()),
                Op::grease(0),
                Op::string(cipherData.data(), cipherData.size()),
                Op::string("\x01\x00", 2),
                Op::begin_scope(),
                Op::grease(2),
                Op::string("\x00\x00", 2),
            ...
            if (proxyAuthState != 0) {
                if (proxyAuthState >= 10) {
                    if (proxyAuthState == 10) {
                        ...
                        // PR #30738: jitter timestamp ±63s to defeat DPI pattern matching
                        int32_t randomJitter = 0;
                        RAND_bytes(reinterpret_cast<unsigned char *>(&randomJitter), sizeof(randomJitter));
                        randomJitter &= 0x3F;
                        ((int32_t *) (tempBuffer->bytes + 64 * 1024 + 28))[0] = old ^ (currentTime + randomJitter);

                        memcpy(tempBuffer->bytes + 11, tempBuffer->bytes + 64 * 1024, 32);
                        bytesRead = 0;

                        // PR #30738: send ClientHello in fragmented chunks with jittered delays
                        // to defeat DPI that can't reassemble fragmented TLS handshakes.
                        bool sendFailed = false;
                        uint32_t offset = 0;
                        // initial random delay 5-100ms
                        {
                            uint32_t initialDelay;
                            RAND_bytes(reinterpret_cast<unsigned char *>(&initialDelay), sizeof(initialDelay));
                            usleep((5000 + (initialDelay % 96000)) );  // 5-100 ms
                        }
                        // first chunk: 1-32 bytes
                        {
                            uint32_t r;
                            RAND_bytes(reinterpret_cast<unsigned char *>(&r), sizeof(r));
                            uint32_t firstChunk = (r % 32) + 1;
                            if (firstChunk > size) firstChunk = size;
                            if (send(socketFd, tempBuffer->bytes, firstChunk, 0) < 0) {
                                if (LOGS_ENABLED) DEBUG_E("connection(%p) send failed (first chunk)", this);
                                closeSocket(1, -1);
                                return;
                            }
                            offset = firstChunk;
                        ...
                        while (offset < size) {
                            // delay 3-25 ms between chunks
                            uint32_t delayR;
                            RAND_bytes(reinterpret_cast<unsigned char *>(&delayR), sizeof(delayR));
                            usleep((3000 + (delayR % 23000)));  // 3-25 ms
                            // chunk size 16-128 bytes
                            uint32_t chunkR;
                            RAND_bytes(reinterpret_cast<unsigned char *>(&chunkR), sizeof(chunkR));
                            uint32_t chunkSize = 16 + (chunkR % 113);  // 16-128
                            if (chunkSize > size - offset) chunkSize = size - offset;
                            if (send(socketFd, tempBuffer->bytes + offset, chunkSize, 0) < 0) {
                                if (LOGS_ENABLED) DEBUG_E("connection(%p) send failed (chunk)", this);
                                closeSocket(1, -1);
                                sendFailed = true;
                                return;
                            }
